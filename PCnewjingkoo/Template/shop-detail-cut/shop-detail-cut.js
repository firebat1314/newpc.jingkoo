myApp.controller('shopDetailCutControl', ['$scope', '$rootScope', '$stateParams', '$state', '$http', 'ipCookie', '$window', '$location', '$anchorScroll', '$sce', function ($scope, $rootScope, $stateParams, $state, $http, ipCookie, $window, $location, $anchorScroll, $sce) {
   ...
   $http({
      ...
      if (res.status) {
         ...
         // console.log($scope.goods_id,$scope.cutting_id)
      }
   })
            ...
            (function (k) {
               $http({
                  ...
                  data: {
                     goods_id: $scope.jingpianListGoodsId,
                     attr: spcArr,
                     qiujing: $scope.arr[k].qiujing,
                     zhujing: $scope.arr[k].zhujing
                  },
               }).success(function (res) {
                  if (res.status) {
                     $scope.arr[k].price = res.data.price.toFixed(2);
                     if (res.data.promotion_id > 0) {
                        $scope.arr[k].youhui = (res.data.promotion_price.substr(1));
                        $scope.arr[k].subprice = ($scope.arr[k].member * $scope.arr[k].youhui).toFixed(2);
                     } else {
                        $scope.arr[k].subprice = ($scope.arr[k].member * $scope.arr[k].price).toFixed(2);
                     }
                  }
               })
            })(k)
   ...
   $scope.add_to_cart_spec = function (success) {
      ...
      if ($scope.jingpianListGoodsId == $scope.goods_id) {
         ...
         parmas = {
            arr_goods: [{
               member: $scope.goodsCarParams.goods.member,
               spec: $scope.goodsCarParams.goods.spec
            }],
