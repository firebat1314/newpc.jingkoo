					...
					if($(this).data('icon') != undefined) {
					 ...
					 if($(this).data('type') != undefined) {
						...
						if(type == 'half') {
							...
							$(this).find('.circle-text-half').css({'line-height': (dimension / 1.45) + 'px', 'font-size' : size + 'px' ,'color':'#fff'});
						} else {
						...
						$(this).append('<span class="circle-text">' + icon + text + '</span>');
						$(this).find('.circle-text').css({'line-height': dimension-12 + 'px', 'font-size' : size + 'px','color':'#fff' });
					}
				...
				if($(this).data('info') != undefined) {
					...
					if($(this).data('type') != undefined) {
						...
						if(type == 'half') { 
							...
							$(this).find('.circle-info-half').css({'line-height': (dimension * 0.9) + 'px', 'color':'#fff' });
						} else {
							$(this).append('<span class="circle-info">' + info + '</span>');
							$(this).find('.circle-info').css({'line-height': (dimension * 1.25) + 'px','color':'#fff' });
						}
					} else {
						$(this).append('<span class="circle-info">' + info + '</span>');
						$(this).find('.circle-info').css({'line-height': (dimension * 1.25) + 'px', 'color':'#fff'});
